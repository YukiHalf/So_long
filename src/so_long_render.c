...
/*   Updated: 2025/09/07 18:08:21 by sdarius-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_tile_area(t_so_long *game, int start_x, int start_y,
		uint32_t color)
{
	int	x;
	int	y;
...
void	draw_tile_border(t_so_long *game, int start_x, int start_y)
{
	...
	while (i < TILE_SIZE && start_x + i < WIDTH)
	{
		...
		if (start_x + i >= 0 && start_y + TILE_SIZE - 1 < HEIGHT)
			mlx_put_pixel(game->image, start_x + i, start_y + TILE_SIZE - 1,
				COLOR_BORDER);
		i++;
	}
	i = 0;
	while (i < TILE_SIZE && start_y + i < HEIGHT)
	{
		...
		if (start_y + i >= 0 && start_x + TILE_SIZE - 1 < WIDTH)
			mlx_put_pixel(game->image, start_x + TILE_SIZE - 1, start_y + i,
				COLOR_BORDER);
		i++;
	}
}
